...
bool MusicStorage::prepareSongDir(QDir &songDir, const QString &hashValue)
{
    ...
    if (musicDir.exists(hashValue))
    {
        ...
        QDir existing = musicDir;
        existing.cd(hashValue);
        existing.removeRecursively();
    }
    if (!musicDir.mkpath(hashValue))
    {
        qWarning() << "[MusicStorage] Couldn't create song directory:" << musicDir.absolutePath() + hashValue;
        return false;
    }
    songDir = musicDir;
        ...
        for (auto &object : objects) {
            ...
            if (songDir != object->storagePath().absoluteFilePath(object->getHash())) {
                ...
                if(!musicDir.mkpath(musicDir.absoluteFilePath(object->getHash())))
                    ...
                    QDir destPath(musicDir.absoluteFilePath(object->getHash()));
...
QVector<std::shared_ptr<MusicObject>> MusicStorage::importMusic(QStringList files)
{
    for (auto &p : m_songs)
    {
    ...
    m_addedFiles.clear();
    convertQueue(queue, musicDir.absoluteFilePath(tmpFolder));

    auto oldDir = QDir(musicDir.absoluteFilePath(tmpFolder));
    ...
    for (auto &file : m_addedFiles)
    {
        ...
        auto newDir = QDir(musicDir.absoluteFilePath(hash));
        ...
        if (player.mediaStatus() == QMediaPlayer::LoadedMedia)
        {
            ...
            obj = std::make_shared<MusicObject>(title.isEmpty() ? info.baseName() : title, duration, artist, musicDir, hash, hash, oldDir);
        }
        else
            obj = std::make_shared<MusicObject>(info.baseName(), 0, "", musicDir, hash, hash, oldDir);

        if (obj->isValid())
        {
            if (!musicDir.mkpath(newDir.absolutePath()))
            {
                qWarning() << "[MusicStorage] Path creation failed";
                continue;
            }
